...
const loadWorkouts = function() {
  ...
  `;
        if (workout.type === "running") html += `<div class="pace-stat">
    ...
    <span>\u{1F9B6} ${workout.cadence}<span>spm</span> </span>
        ...
        if (workout.type === "cycling") html += `<div class="pace-stat">
...
const addMarker = function(lat, lng, type) {
    const marker = L.marker([
        ...
        autoClose: false,
        closeOnClick: false
    })).setPopupContent(type === "running" ? "\uD83C\uDFC3 Corrida" : "\uD83D\uDEB4 Pedalada").openPopup();
};
...
const createWorkout = function(clickLat, clickLng) {
    if (inputType.value === "running") {
        const running = new Running([
            clickLat,
            clickLng
        ], inputDuration.value, inputDistance.value, inputCadence.value);
